...
- `app/api/nankai-reasoning/route.ts`, `lib/nankai-reasoning.ts`, and `tests/nankai-reasoning.test.ts` — strict Sol three-world contract, prompt-injection containment, duplicate-world rejection, deterministic consequence ranking, recommendation withholding, and no automatic world application.
